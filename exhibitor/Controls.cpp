...
void CONTROLS_EmptyListViewControl(HWND hcontrol)
{
...
void CONTROLS_GetListViewText(HWND hcontrol,int item,int subitem,char * buf,int buflen)
{
  ListView_GetItemText(hcontrol,item,subitem,buf,buflen);
}
                                  ...
                                  TCHAR *        WindowName )
{
  ...
  for(i=0;i<nColumns;i++)
  {
    ColumnsSizes[i]=((((rectangle.right-rectangle.left)*WidthPercent)/100)*ColumnsSizes[i])/100;
...
unsigned int CONTROLS_ListViewNotifyCallback(UINT uMsg,WPARAM wParam,LPARAM lParam,LPNMHDR pnmhdr,PCOMMON_CONTROL_NOTIFICATION_INFO pinfo)
{
  ...
  switch(pnmhdr->code)
  {
    case NM_KILLFOCUS:
    {
      code = CONTROL_LISTVIEW_KILLFOCUS_NOTIFICATION;
    }
    break;
